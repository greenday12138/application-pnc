...
bool IsNonmovableObstacle(const ReferenceLineInfo& reference_line_info,
  ...
  for (const auto* other_obstacle :
    ...
    if (other_boundary.start_s() < this_boundary.start_s() &&
        other_boundary.end_s() >
            this_boundary.start_s() - kObstaclesDistanceThreshold) {
      AINFO << obstacle.Id() << " is blocked by " << other_obstacle->Id();
      return false;
    }
  }
